...
## Running on AWS with S3 (optional, for larger runs)

You can run the same pipeline on an **AWS EC2 instance** and use **Amazon S3** to store the collected PDFs and metadata. The core logic (discover → score → export → fetch) stays the same – only the environment and storage change.

- **Compute**: 1 EC2 instance (e.g., `t3.medium`) with:
  - Python 3, git, and internet access.
  - This repository cloned and installed with `requirements.txt` (same as local setup).

- **Database**:
  - For experiments, a local SQLite file `data/uwss.sqlite` on the EC2 instance is sufficient.
  - Later you can point UWSS to a PostgreSQL database (e.g., AWS RDS) via `--db-url` / `UWSS_DB_URL`.

- **Storage**:
  - JSONL exports and downloaded PDFs live under `data/` on the EC2 instance.
  - A dedicated S3 bucket (e.g. `uwss-corrosion-data`) is used to store the **final PDFs and optional sidecar metadata**.

### Example: minimal EC2 + S3 setup

1. **On AWS**:
   - Create an S3 bucket, e.g. `uwss-corrosion-data`.
   - Create an IAM Role for EC2 with permission to write to that bucket (e.g. `s3:PutObject` on `uwss-corrosion-data/*`). Attach this role to your EC2 instance.

2. **On the EC2 instance** (SSH in):
...
# Copy or edit config/config.yaml as on your local machine (keep domain_keywords).

python -m src.uwss.cli db-init --db data/uwss.sqlite

# Run a standard pipeline on EC2, e.g.:
python -m src.uwss.cli crossref-lib-discover   --config config/config.yaml --db data/uwss.sqlite --max 200
python -m src.uwss.cli openalex-lib-discover   --config config/config.yaml --db data/uwss.sqlite --max 200
python -m src.uwss.cli semantic-scholar-lib-discover --config config/config.yaml --db data/uwss.sqlite --max 80
python -m src.uwss.cli paperscraper-discover   --config config/config.yaml --db data/uwss.sqlite --source pubmed --max 200
python -m src.uwss.cli paperscraper-discover   --config config/config.yaml --db data/uwss.sqlite --source arxiv  --max 200

python -m src.uwss.cli score-keywords --config config/config.yaml --db data/uwss.sqlite

python -m src.uwss.cli export \
  --db data/uwss.sqlite \
  --out data/corrosion_papers_cloud.jsonl \
  --require-match \
  --min-score 0.5 \
  --require-abstract \
  --min-abstract-length 80
...
3. **Upload PDFs (and optional metadata) to S3** using the built‑in `s3-upload` command (see `src/uwss/upload.py`):

```bash
python -m src.uwss.cli s3-upload \
  --db data/uwss.sqlite \
  --files data/files \
  --bucket uwss-corrosion-data \
  --prefix "runs/2025-11-20/" \
  --include-sidecars \
  --include-docjson
```

This will push the PDFs referenced by `Document.local_path` – and, optionally, their sidecar `.meta.json` and per-document `.doc.json` – into your S3 bucket under the chosen prefix. You can then compare:
- Local vs EC2 **runtime and throughput**.
- Local files vs S3 bucket contents (number of PDFs, size, etc.).
