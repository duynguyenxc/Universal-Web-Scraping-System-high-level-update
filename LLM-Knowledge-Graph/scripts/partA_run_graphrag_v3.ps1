Param(
  [Parameter(Mandatory=$false)][string]$OutDir = "graphrag-project/output_partA_v3",
  [Parameter(Mandatory=$false)][string]$InputDir = "graphrag-project/input_partA_v3",
  ...
  [Parameter(Mandatory=$false)][string]$Settings = "graphrag-project/settings.partA.v3.yaml",
...
Write-Host "== Part A v3: metadata -> input -> GraphRAG index/query ==" -ForegroundColor Cyan
...
$env:PARTA_CACHE_DIR = (Join-Path $env:TEMP "graphrag_cache_partA_v3")
...
# 8) Verification audit report (v3 output; avoid overwriting stable artifact)
python "scripts/partA_audit_outputs.py" --out-dir "$OutAbs" --out-md "artifacts/partA/verification_audit_v3.md"

# 9) Verification-grade exports (v3 output; avoid overwriting stable artifact)
if (Test-Path (Join-Path $OutAbs "entities.parquet")) {
  python "scripts/partA_export_cmo_configurations.py" --out-dir "$OutAbs" --out-claims-md "artifacts/partA/claims_enriched_v3.md" --out-md "artifacts/partA/cmo_configurations_v3.md"
}

# 10) v3 share page / bundle (does NOT overwrite v1/v2 share)
python "scripts/partA_create_run_bundle_v3.py" --out-dir "$OutAbs" --artifacts-dir "artifacts/partA" --mode "share"
