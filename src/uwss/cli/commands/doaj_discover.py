"""CLI command for DOAJ discovery."""
...
from ...sources.doaj import discover_doaj
...
def register(sub) -> None:
    """Register the doaj-discover command."""
    p = sub.add_parser(
        "doaj-discover",
        help="Discover papers from DOAJ via OAI-PMH",
    )
    ...
    p.add_argument(
        ...
        type=float,
        default=1.0,
        help="Delay between requests (seconds). DOAJ recommends 1.0+ for polite use.",
    )
    p.add_argument(
        "--articles-only",
        action="store_true",
        help="Only discover articles (exclude journals)",
    )
    p.add_argument(
        "--journals-only",
        action="store_true",
        help="Only discover journals (exclude articles)",
    )
    p.add_argument(
        "--metrics-out",
        default=None,
    ...
    def _cmd(args: argparse.Namespace) -> int:
        """Execute DOAJ discovery command."""
        ...
        try:
            ...
            include_articles = not args.journals_only
            include_journals = not args.articles_only
            
            console.print(f"[cyan]Starting DOAJ discovery (articles={include_articles}, journals={include_journals})...[/cyan]")

            for metadata in discover_doaj(
            ...
            console.print(
                f"[green]DOAJ Discovery: inserted={inserted} duplicates={duplicates} failed={failed} elapsed={elapsed_sec:.1f}s[/green]"
            )
